...
import { NftDegreeMeta, PinataRes } from '@_types/nft';
...
const NftDegreeCreate: NextPage = () => {
  const {ethereum, contract} = useWeb3();
  const {network} = useNetwork();
  const [nftURI, setNftURI] = useState("");
  const [price, setPrice] = useState("");
  const [hasURI, setHasURI] = useState(false);
  const [NftDegreeMeta, setNftMeta] = useState<NftDegreeMeta>({
    name: "",
    description: "",
    image: "",
    attributes: [
      { degree_trait_type:"Issued to", value: ""},
      { degree_trait_type:"Issue Date", value: ""},
      { degree_trait_type:"Course Type", value: ""},
    ]
  });
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    setNftMeta({...NftDegreeMeta, [name]: value});
  }

  const handleAttributeChange = (e: ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    const attributeIdx = NftDegreeMeta.attributes.findIndex(attr => attr.degree_trait_type === name);

    NftDegreeMeta.attributes[attributeIdx].value = value;
    setNftMeta({
      ...NftDegreeMeta,
      attributes: NftDegreeMeta.attributes
    })
  }
              ...
              <div className="shadow sm:rounded-md sm:overflow-hidden">
                <div className="px-4 py-5 bg-white space-y-6 sm:p-6">
                  ...
                  <div>
                    ...
                    <div className="mt-1">
                      <textarea
                        value={NftDegreeMeta.description}
                        onChange={handleChange}
                        id="description"
                        name="description"
                        rows={3}
                  ...
                  {/* Has Image? */}
                  { NftDegreeMeta.image ?
                    <img src={NftDegreeMeta.image} alt="" className="h-40" /> :
                    <div>
                    <label className="block text-sm font-medium text-gray-700">Add  Certificate/Degree</label>
                  ...
                  <div className="grid grid-cols-6 gap-6">
                    { NftDegreeMeta.attributes.map(attribute =>
                      <div key={attribute.degree_trait_type} className="col-span-6 sm:col-span-6 lg:col-span-2">
                        <label htmlFor={attribute.degree_trait_type} className="block text-sm font-medium text-gray-700">
                        ...
                        <input
                          ...
                          type="text"
                          name={attribute.degree_trait_type}
                          id={attribute.degree_trait_type}
...
export default NftDegreeCreate
